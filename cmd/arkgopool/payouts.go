...
func SendPayments(silent bool) {
	...
	for _, element := range votersEarnings {
		...
		if txAmount2Send > minAmountSetting && txAmount2Send > 0 {
			tx := core.CreateTransaction(element.Address, txAmount2Send, viper.GetString("voters.txdescription"), p1, p2, feeAmount)
	...
	if costAmount2Send > 0 {
		...
		txCosts := core.CreateTransaction(costAddress, costAmount2Send, viper.GetString("costs.txdescription"), p1, p2, feeAmount)
	...
	if reserveAmount2Send > 0 {
		...
		txReserve := core.CreateTransaction(reserveAddress, reserveAmount2Send, viper.GetString("reserve.txdescription"), p1, p2, feeAmount)
	...
	if personalAmount2Send > 0 {
		...
		txpersonal := core.CreateTransaction(personalAddress, personalAmount2Send, viper.GetString("personal.txdescription"), p1, p2, feeAmount)
...
func SendBonusPayment(iAmount int, txDesc string) {
	...
	for _, element := range voters.Accounts {
		...
		//transaction parameters
		tx := core.CreateTransaction(element.Address, txAmount2Send, txDesc, p1, p2, feeAmount)
		payload.Transactions = append(payload.Transactions, tx)
		//Logging history to DB
